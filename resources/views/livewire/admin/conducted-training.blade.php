<div>
    ...
    <table cellpadding="2">
        ...
        <tbody>
            @if (count($conducted_trainings) > 0)
                @foreach ($conducted_trainings as $training)
                <tr>
                    ...
                    <td data-label="Status">
                        @if ($training->status == 0)
                            <span class="bg-warning">Ongoing</span>
                        @else
                            <span class="bg-success">Finished</span>
                        @endif
                    </td>
                    <td data-label="Action" class="d-block py-2">
                        <button class="btn btn-info" id="{{ $training->id }}" data-bs-toggle="modal" data-bs-target="#edit_training_modal{{ $training->id }}"><i class="bi bi-eye"></i></button>
                    </td>
                </tr>
                @endforeach
            @else
                <tr>
                    <td data-label="Training" colspan="6" class="text-center p-2">No Training Conducted</td>
                </tr>
            @endif
        </tbody>
    </table>
    <span class="fs-5">Total: {{ count($conducted_trainings) }}</span>
</div>

@foreach ($conducted_trainings as $training)
     <!-- View Conducted Training Modal-->
     <div class="modal fade" id="edit_training_modal{{ $training->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title text-white" id="exampleModalLabel">Conducted Training</h5>
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="border bg-transparent p-2 mb-2 w-100">{{ $training->training_id }}</div>
                        </div>
                        <div class="col-md-6">
