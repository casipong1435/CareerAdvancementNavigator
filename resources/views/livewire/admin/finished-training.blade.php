    ...
    <div class="py-2 d-flex justify-content-between">
        ...
        <span class="filter d-flex flex-row d-inline-block">
            <div class="mx-2 d-flex justify-content-center align-items-center">
                ...
                <select class="mx-1" wire:model.live="type_of_training" name="orderBy">
                    <option disabled>Training Type</option>
            ...
            @if (count($official_trainings) > 0)
                    @foreach ($official_trainings as $official_training)
                        <tr>
                            ...
                            <td data-label="Training Type">{{ $official_training->training_type }}</td>         
                              ...
                              <div class="modal-content">
                                ...
                                <div class="modal-body p-4">
                                    <div class="container">
                                        ...
                                        <div class="row">
                                              <div class="col-md-6">
                                                <span class="fw-bold">Started of Training</span>
                                                ...
                                                <div class="border bg-transparent p-2 mb-2 w-100">{{ $official_training->responsible_unit }}</div>
                                                <span class="fw-bold">No. of Participants</span> 
                                                <div class="border bg-transparent p-2 mb-2 w-100">{{ $official_training->number_of_participants }}</div>
                                              </div>
                                              <div class="col-md-6">
                                                ...
                                                <div class="border bg-transparent p-2 mb-2 w-100">{{ $official_training->venue }}</div>
                                              </div>
                                              <div class="col-md-12">
                                                <span class="fw-bold">Training Type</span>
                                                <div class="border bg-transparent p-2 mb-2 w-100">{{ $official_training->training_type }}</div>
                                              </div>
                                          </div>
                                    </div>
                                </div>
                                @php
                                    $training_id = [
                                        'id' => Crypt::encrypt($official_training->training_id)
                                    ]
                                @endphp
                                <div class="modal-footer">
                                    <a href="{{ route('ViewTrainingParticipant', $training_id) }}" class="btn btn-primary">View Participants</a>
                ...
                @else
                    <tr>
                        <td data-label="Training" colspan="7" class="text-center">No Training Added Yet.</td>
                    </tr>
                @endif
        </tbody>
    </table>
            ...
            <div class="modal-body p-4">
                <div class="col-md-12 p-2 mb-1">
                    <div class="form-group mb-2">
                        <select class="p-2 w-100" wire:model.change="report">
                            <option disabled value="">Select Report</option>
                            <option value="GAD">GAD Report</option>
                            <option value="Conducted">Conducted Report</option>
                        </select>
                    </div>
                ...
                @if (session()->has('found-gad'))
                    <div class="col-md-12 px-3">
                        <div class="alert alert-success fade show alert-dismissable position-relative">
                            <strong>Found! </strong> {{ session()->get('found-gad') }}
                            <a target="_blank" href="{{ route('printGADReport', ['from' => $from, 'to' => $to]) }}" class=" text-decoration-none p-2 rounded position-absolute" style="color: #5461D4; top: 8px; right:17px"><i class="bi bi-file-earmark-arrow-down-fill fs-3"></i></a>
                        </div>
                    </div>
                @endif
                @if (session()->has('found-conducted'))
                    <div class="col-md-12 px-3">
                        <div class="alert alert-success fade show alert-dismissable position-relative">
                            <strong>Found! </strong> {{ session()->get('found-conducted') }}
                            <a target="_blank" href="{{ route('printConductedTraining', ['from' => $from, 'to' => $to]) }}" class=" text-decoration-none p-2 rounded position-absolute" style="color: #5461D4; top: 8px; right:17px"><i class="bi bi-file-earmark-arrow-down-fill fs-3"></i></a>
                        </div>
                    </div>
                @endif
                @if (session()->has('fail'))
